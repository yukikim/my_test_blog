...
export default function SkillMap({ title = "Skills", skills = defaultSkills }: Props) {
  return (
    <section>
      <h2 className="mb-4 text-xl font-semibold text-zinc-200">{title}<br /><span className="text-sm font-normal text-zinc-200">習得したスキル(習得度は大凡の値です)</span></h2>
      <ul className="space-y-3 text-zinc-300">
        {skills.map((s) => (
          <li key={s.name} className="rounded p-2 content-frame-in mb-1">
            <div className="flex items-center justify-between">
              <span className="font-medium text-sm">{s.name}</span>
              <span className="text-xs text-zinc-200">Lv.{s.level}{s.years ? ` / ${s.years} yrs` : ""}</span>
            </div>
